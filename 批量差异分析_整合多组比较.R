run_batch_de_mixed <- function(
  ...
  label_genes = NULL, # NULL：沿用自动标注；非 NULL：只标注指定基因
  plot_width = 10,
  plot_height = 8,
  plot_dpi = 300,
  ...
  plot_heatmap_multi_group_overall <- function(
    ...
    if (
      is.null(rownames(sample_info)) || !group_col %in% colnames(sample_info)
    ) {
    ...
    if (nrow(sig_tbl) == 0) {
      warning("多组整体比较没有显著基因，跳过热图。")
      return(invisible(NULL))
    }

    if (!is.null(top_n) && is.finite(top_n) && nrow(sig_tbl) > top_n) {
      sort_col <- if ("F" %in% colnames(sig_tbl)) "F" else pcol
      decreasing <- identical(sort_col, "F")
      sig_tbl <- sig_tbl[
        order(sig_tbl[[sort_col]], decreasing = decreasing),
        ,
        drop = FALSE
      ]
    ...
    ann_df <- data.frame(Group = grp_fac, row.names = samples_used)
    ...
    my_palette <- grDevices::colorRampPalette(c("#045a8d", "white", "#d7301f"))(
      1000
    )
  ...
  multi_group_limma_de <- function(
    ...
    list(
      ...
      method = "limma_F_test",
      heatmap_matrix = if (
        !is.null(hm_out) && !is.null(hm_out$heatmap_matrix)
      ) {
      ...
      group_sizes = stats::setNames(as.integer(table(grp_raw)[groups]), groups)
    )
  }

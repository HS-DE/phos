...
#   This script does not infer kinase names from enrichment output and does not
#   guess custom annotation columns. If a custom annotation data.frame is used,
#   annotation_kinase_col and annotation_site_col must be explicitly provided.
#
...
.safe_wilcox_p <- function(x, y, alternative) {
  ...
  out <- tryCatch(
    suppressWarnings(stats::wilcox.test(x, y, alternative = alternative, exact = FALSE)$p.value),
...
normalize_kinase_annotation <- function(annotation,
  ...
  # Case 1: PhosR default annotation is a named list:
  #   names(annotation) = kinase names
  #   annotation[[kinase]] = substrate site labels, e.g. GENE;S123;
  if (is.list(annotation) && !is.data.frame(annotation)) {
    ...
    # Case 2: custom data.frame annotation.
    # No guessing is allowed here. User must explicitly state which columns are used.
    ...
    if (!annotation_kinase_col %in% colnames(anno_df)) {
      stop("annotation_kinase_col 不在 annotation 中：", annotation_kinase_col)
    }
    if (!annotation_site_col %in% colnames(anno_df)) {
      stop("annotation_site_col 不在 annotation 中：", annotation_site_col)
    }
...
run_one_rank_based_ksea <- function(stats_tbl,
  ...
  if (nrow(stats_tbl) < min_sites) {
    ...
    return(list(comparison = comparison, input_stats = stats_tbl, signed_score = data.frame(), plot_files = character(0)))
  }
  ...
  list(comparison = comparison, input_stats = stats_tbl, signed_score = signed_score, plot_files = plot_files)
}
...
run_phosphosite_ksea <- function(de_results,
  ...
  site_anno <- build_ksea_site_annotation(site_mat2 = site_mat2, keep_only_single_site = keep_only_single_site, uppercase_gene = uppercase_gene)
  ...
  kinase_name_check <- annotation_tbl %>% count(Kinase, name = "n_substrate_sites") %>% arrange(Kinase)

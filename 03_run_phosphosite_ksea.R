...
run_one_rank_based_ksea <- function(stats_tbl,
  ...
  utils::write.csv(stats_tbl, file.path(comp_dir, paste0(comp_slug, "_KSEA 磷酸化位点 logFC 向量表.csv")), row.names = FALSE)
  ...
  utils::write.csv(enr_up_n, file.path(comp_dir, paste0(comp_slug, "_KSEA 上调方向富集结果.csv")), row.names = T)
  utils::write.csv(enr_down_n, file.path(comp_dir, paste0(comp_slug, "_KSEA 下调方向富集结果.csv")), row.names = T)
  utils::write.csv(signed_score, file.path(comp_dir, paste0(comp_slug, "_KSEA 激酶活性趋势评分表.csv")), row.names = FALSE)
...
run_phosphosite_ksea <- function(de_results,
  ...
  utils::write.csv(site_anno, file.path(out_dir, "01_KSEA 磷酸化位点注释表.csv"), row.names = FALSE)
  ...
  utils::write.csv(de2, file.path(out_dir, "02_带 KSEA 位点标签的磷酸化差异结果表.csv"), row.names = FALSE)
  ...
  utils::write.csv(match_summary, file.path(out_dir, "02_KSEA 位点标签匹配汇总表.csv"), row.names = FALSE)
  ...
  utils::write.csv(signed_long, file.path(out_dir, "KSEA 激酶活性趋势评分长表.csv"), row.names = FALSE)
  ...
  if (nrow(signed_long) > 0) {
    ...
    utils::write.csv(signed_wide, file.path(out_dir, "KSEA 激酶活性趋势评分表.csv"), row.names = FALSE)
